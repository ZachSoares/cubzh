...
bool shape_ray_cast(const Transform *t,
    ...
    Ray *modelRay = ray_world_to_local(worldRay, &model);
...
bool shape_point_overlap(const Shape *s, const float3 *world) {
    ...
    Matrix4x4 model;
    transform_utils_get_model_ltw(s->transform, &model);

    float3 modelPoint;
    matrix4x4_op_multiply_vec_point(&modelPoint, world, &model);

    Chunk *c;
    const SHAPE_COORDS_INT3_T coords_in_shape = (SHAPE_COORDS_INT3_T){(SHAPE_COORDS_INT_T)modelPoint.x,
